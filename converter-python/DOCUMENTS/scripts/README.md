...
```bash
# Run against local uvicorn (default — starts python3 -m uvicorn on a test port)
...
PORT=19000 ./scripts/run-integration-tests.sh --python
...
./scripts/run-integration-tests.sh --python --keep
...
```bash
make test-integration          # starts local uvicorn + runs full suite
make test-integration-docker   # builds Docker image + runs full suite inside container
make generate-samples          # creates testdata/sample.xlsx and sample.ods
...
- `curl`
- `python3` (used for JSON parsing when `jq` is absent; also the runtime for `--python` mode)
- `soffice` (LibreOffice) — only needed for `generate-samples.sh` and local uvicorn tests
- `jq` is optional but nice to have (`apt install jq`)

The test scripts are self-contained and do not depend on any test framework.
