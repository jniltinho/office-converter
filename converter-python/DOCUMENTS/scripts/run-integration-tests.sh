...
# Starts the office-converter (binary, php built-in server, or docker), waits for healthy,
...
#   ./scripts/run-integration-tests.sh --php
...
# Runner lives under DOCUMENTS/scripts/. Real project root is two levels up.
...
Options:
  ...
  --python            Run tests against the local uvicorn server (python3 -m uvicorn)
...
MODE_DESC="$( [[ -n "$EXTERNAL_URL" ]] && echo "external server at $SERVER_URL" || echo "managed server on :$TEST_PORT" )"
if [[ $USE_PYTHON -eq 1 ]]; then MODE_DESC="$MODE_DESC (uvicorn)"; fi
echo "    mode: $MODE_DESC"
echo "    docker: $USE_DOCKER  python: $USE_PYTHON"
echo
...
start_docker_server() {
  ...
  docker rm -f xlsb-test 2>/dev/null || true
  # The container's ENTRYPOINT (/app/entrypoint.sh) starts FrankenPHP in
  # worker mode automatically. We just need to pass the port/host via
  # the OFFICE_* env vars that the entrypoint understands.
  docker run -d --name xlsb-test \
    -p "${TEST_PORT}:${TEST_PORT}" \
    -e OFFICE_PORT="${TEST_PORT}" \
    -e OFFICE_HOST=0.0.0.0 \
    office-converter:latest >/dev/null
  need_cleanup=1
}

start_python_server() {
  echo "==> Starting uvicorn server on 127.0.0.1:$TEST_PORT (background)"
  (cd "$ROOT_DIR" && OFFICE_PORT=$TEST_PORT python3 -m uvicorn app.main:app \
    --host 127.0.0.1 --port ${TEST_PORT} \
    > "$ROOT_DIR/test-server.log" 2>&1) &
  server_pid=$!
  need_cleanup=1
  echo "    pid: $server_pid (log: test-server.log)"
}
