"""
Embedded drag-and-drop web UI served at ``GET /``.

Keeping the HTML in a dedicated module prevents it from cluttering the route
handlers in ``routes.py``.  The page is fully self-contained (no external
assets) and auto-detects the source format from the file extension to select
the correct typed conversion endpoint.
"""
