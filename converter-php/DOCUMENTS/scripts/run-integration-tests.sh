...
Options:
  ...
  --php               Run tests against the local PHP built-in server (php -S api/router.php)
...
if [[ $USE_PHP -eq 1 ]]; then MODE_DESC="$MODE_DESC (php -S)"; fi
echo "    mode: $MODE_DESC"
echo "    docker: $USE_DOCKER  php: $USE_PHP"
echo
